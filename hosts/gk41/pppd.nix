            ...
            { name, ... }:
            {
              options = {
                ...
                configFile = mkOption {
                  ...
                  default = null;
                  example = "/run/secrets/ppp/home";
                  description = ''
                    The path of pppd peer configuration file.
                  '';
                };
              };
            }
        ...
        value = {
          ...
          serviceConfig =
            ...
            in
            {
              Type = "notify";
              ExecStart =
                if peerCfg.configFile != null then
                  "${getBin cfg.package}/sbin/pppd file ${peerCfg.configFile} up_sdnotify nolog"
                else
                  "${getBin cfg.package}/sbin/pppd call ${peerCfg.name} up_sdnotify nolog";
              Restart = "always";
              RestartSec = 5;
